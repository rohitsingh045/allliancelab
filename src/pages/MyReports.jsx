...
const MyReports = () => {
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="max-w-lg mx-auto px-4 py-20 text-center">
          ...
          <p className="text-muted-foreground mb-6">Please log in to view your reports.</p>
  ...
  const statusLabel = {
    placed: { text: "Placed", cls: "bg-amber-100 text-amber-700" },
    confirmed: { text: "Confirmed", cls: "bg-sky-100 text-sky-700" },
    sample_collected: { text: "Sample Collected", cls: "bg-indigo-100 text-indigo-700" },
    processing: { text: "Processing", cls: "bg-violet-100 text-violet-700" },
        ...
        {loading ? (
          ...
          <div className="bg-card rounded-2xl border border-border p-12 text-center shadow-sm">
            ...
            <p className="text-muted-foreground mb-6 max-w-md mx-auto">
              Reports will appear here once the lab uploads your test results. Book a test to get started!
            </p>
            <Button onClick={() => navigate("/")} className="bg-gradient-primary text-white gap-2">
              <FlaskConical className="w-4 h-4" /> Book a Test
            </Button>
          </div>
        ) : (
                      ...
                      <p className="text-sm text-muted-foreground">
                        ...
                        {reportsReady === totalItems ? (
                          <span className="text-emerald-600 font-medium">
                            All {reportsReady} report{reportsReady !== 1 ? "s" : ""} ready
                          </span>
                        ) : reportsReady > 0 ? (
                          <span className="text-amber-600 font-medium">
                            {reportsReady}/{totalItems} reports ready
                          </span>
                        ) : (
                          <span className="text-muted-foreground">Reports pending</span>
                        )}{" "}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground capitalize">
                              ...
                              {item.hasReport && item.reportUploadedAt && (
                                <> &bull; Uploaded {new Date(item.reportUploadedAt).toLocaleDateString()}</>
                              )}
                            </p>
                          </div>
