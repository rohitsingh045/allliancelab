...
import {
  ...
  FileUp, Trash2, ClipboardCopy, Hash, Home, MapPin, Plus, Briefcase
} from "lucide-react";
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (activeTab === "job-applications" && jobApplications.length === 0) {
      fetchJobApplications();
    }
  }, [activeTab, testsList.length, jobApplications.length]);
  ...
  const fetchJobApplications = async () => {
    setLoadingApps(true);
    try {
      const res = await fetch(`${API_BASE}/job-applications`, { headers });
  ...
  const updateAppStatus = async (appId, status) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/job-applications/${appId}`, {
        method: "PATCH",
        headers,
      ...
      if (res.ok) {
        ...
        setJobApplications((prev) => prev.map((a) => (a._id === appId ? updated : a)));
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const deleteApp = async (appId) => {
    ...
    try {
      ...
      if (res.ok) setJobApplications((prev) => prev.filter((a) => a._id !== appId));
    } catch (err) {
      console.error(err);
    } finally {
      setDeletingApp(null);
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 relative z-10">
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-all flex items-center gap-1.5 ${
              activeTab === "job-applications"
                ...
                : "bg-white/80 backdrop-blur-sm text-purple-700 hover:bg-white border border-purple-100"
            }`}
          >
            <Briefcase className="w-3.5 h-3.5" /> Job Applications
            {jobApplications.filter((a) => a.status === "new").length > 0 && (
              <span className="ml-1 bg-rose-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">
                {jobApplications.filter((a) => a.status === "new").length}
              </span>
            )}
          </button>
          <button
        ...
        {activeTab === "job-applications" && (
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-purple-100 shadow-sm">
            <div className="p-5 border-b border-purple-50 flex items-center gap-2">
              <Briefcase className="w-5 h-5 text-purple-500" />
              <h2 className="font-bold text-lg text-slate-800">Job Applications</h2>
              <span className="ml-2 text-xs text-slate-500">{jobApplications.length} total</span>
              <button
                onClick={fetchJobApplications}
                className="ml-auto px-3 py-1.5 rounded-lg bg-purple-50 text-purple-600 hover:bg-purple-100 text-xs font-medium transition-colors flex items-center gap-1"
              >
            ...
            {loadingApps ? (
              ...
              <div className="text-center py-16">
                <Briefcase className="w-12 h-12 text-purple-200 mx-auto mb-3" />
                <p className="text-purple-400 font-medium">No job applications yet</p>
                <p className="text-slate-400 text-xs mt-1">Applications submitted via the website ad will appear here</p>
              </div>
            ) : (
              <div className="divide-y divide-purple-50">
                {jobApplications.map((app) => (
                  <div key={app._id} className="p-5 hover:bg-purple-50/30 transition-colors">
                    <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                      {/* Avatar + name */}
                      <div className="flex items-center gap-3 flex-1">
                        <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-purple-400 to-indigo-500 flex items-center justify-center text-white font-bold text-lg shrink-0 shadow-sm">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-0.5 flex-wrap">
                            <span className="flex items-center gap-1 text-xs text-slate-500">
                              ...
                              <a href={`tel:${app.phone}`} className="hover:text-purple-600 transition-colors">{app.phone}</a>
                            </span>
                            {app.email && (
                              <span className="flex items-center gap-1 text-xs text-slate-500">
                      ...
                      <div className="flex flex-col gap-2 sm:items-end shrink-0">
                        ...
                        <span className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize ${
                          app.status === "new" ? "bg-blue-100 text-blue-800 border border-blue-200" :
                          app.status === "reviewed" ? "bg-yellow-100 text-yellow-800 border border-yellow-200" :
                          app.status === "shortlisted" ? "bg-emerald-100 text-emerald-800 border border-emerald-200" :
                          "bg-rose-100 text-rose-800 border border-rose-200"
                        }`}>
                          ● {app.status}
                        </span>
                    ...
                    {(app.experience || app.message) && (
                      <div className="mt-3 ml-14 grid sm:grid-cols-2 gap-2">
                        {app.experience && (
                          <div className="bg-slate-50 rounded-lg px-3 py-2 border border-slate-100">
                            <p className="text-[10px] text-slate-400 uppercase font-semibold tracking-wide mb-0.5">Experience</p>
                            <p className="text-xs text-slate-700">{app.experience}</p>
                          </div>
                        )}
                        {app.message && (
                          <div className="bg-slate-50 rounded-lg px-3 py-2 border border-slate-100">
                            <p className="text-[10px] text-slate-400 uppercase font-semibold tracking-wide mb-0.5">Message</p>
                            <p className="text-xs text-slate-700">{app.message}</p>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    <div className="mt-3 ml-14 flex items-center gap-2 flex-wrap">
                      ...
                      {["new", "reviewed", "shortlisted", "rejected"].map((s) => (
                        <button
                          key={s}
                          onClick={() => updateAppStatus(app._id, s)}
                          disabled={app.status === s || updatingApp === app._id}
                        ...
                        >
                          {updatingApp === app._id ? "…" : s}
                        </button>
                      ))}
                      <button
                        onClick={() => deleteApp(app._id)}
                        disabled={deletingApp === app._id}
                        className="ml-auto flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-medium text-rose-600 hover:bg-rose-50 border border-rose-100 transition-colors disabled:opacity-50"
                      >
                        <Trash2 className="w-3 h-3" />
                        {deletingApp === app._id ? "Deleting…" : "Delete"}
                      </button>
                    </div>
                  </div>
                ))}
